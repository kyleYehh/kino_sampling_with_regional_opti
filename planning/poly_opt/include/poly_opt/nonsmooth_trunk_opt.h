...
public:
  // store latest cost
  vector<double> latest_seg_cost_;
  vector<double> latest_Ts_;
                      ...
                      const vector<int> &children_descendant_num_plus_one) 
  {
    ...
    latest_seg_cost_.resize(children_size_ + 1);
    latest_Ts_.resize(children_size_ + 1);
...
private:
                  ...
                  const int n)
  {
    ...
    obj += calTimeItgPntAddGrad(state_p_, D, Tau[children_size_], g_D, g_Tau[children_size_], AS_CHILD, self_descendant_num_plus_one_);
    ...
    for (int i=0; i<children_size_; ++i)
    {
      ...
      obj += calTimeItgPntAddGrad(D, state_c_[i], Tau[i], g_D, g_Tau[i], AS_PARENT, children_descendant_num_plus_one_[i]);
    }
    if (g != nullptr)
    {
  ...
  double calTimeItgPntAddGrad(const VectorXd &x0, const VectorXd &x1, double Tau, Eigen::Map<VectorXd> grad_d, double &grad_Tau, bool type, double weight = 1) 
  {
    ...
    for (int j = 0; j <= K_pos; ++j, t1 += step_pos) 
    {
      ...
      //   grad_Jv_to_t += omega * (cost_tmp / K_ + step_pos * grad_Jv_to_v.transpose() * grad_v_to_t);
      ...
      //   grad_Ja_to_t += omega * (cost_tmp / K_ + step_pos * grad_Ja_to_a.transpose() * grad_a_to_t);
    ...
    double cost_pos1(0.0), cost_vel1(0.0), cost_acc1(0.0);
    ...
    Matrix<double, 3, 3> grad_J_to_d = step_pos * grad_Jp_to_d * rho_pos_ 
                                      + step_higher_deri * (grad_Jv_to_d * rho_vel_ + grad_Ja_to_d * rho_acc_);
    grad_J_to_d *= weight;
    ...
    double cost(step_pos * cost_pos * rho_pos_ 
                + step_higher_deri * (cost_vel * rho_vel_ + cost_acc * rho_acc_));
    cost *= weight;
    return cost;
  }
